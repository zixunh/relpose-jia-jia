...
# SUBSET_CATEGORIES = ["apple"] #dev
SET_LISTS = ["manyview_dev_0", "manyview_dev_1"]
...
def process_poses(co3d_dir, category, output_dir, min_quality):
    ...
    subset_lists_data = {'train':[],'test':[],'val':[]}
    for set_name in SET_LISTS:
        subset_lists_file = osp.join(category_dir, "set_lists/set_lists_"+set_name+".json")
        with open(subset_lists_file) as f:
            data = json.load(f)
        for k in data.keys():
            subset_lists_data[k]+=data[k]
            # print(k, len(subset_lists_data[k]), len(data[k]))
