...
def get_parser():
    ...
    parser.add_argument("--output_dir", type=str, default="data/co3d_v1_annotations")
    parser.add_argument("--co3d_v1_dir", type=str, default="data/co3d_v1")
    parser.add_argument(
        "--min_quality",
        type=float,
        default=0.5,
...
def process_poses(co3d_dir, category, output_dir, min_quality):
    ...
    subset_lists_file = osp.join(category_dir, "set_lists.json")
    ...
    for subset in ["train_known", "test_known", "train_unseen", "test_unseen"]:
        ...
        output_file = osp.join(args.output_dir, f"{category}_{subset}.jgz")
        with gzip.open(output_file, "w") as f:
